...
# DFredin, 2022-Dec-10, Modified file to handle tracks on CDs as well as csv files
#------------------------------------------------------#
...
class CD:
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        if not str(track_id).isnumeric():
            raise Exception('Track ID must be an integer!')
        else:
